...
def unpack_bootimg(boot_img, output_dir):
    ...
    with open(boot_img, 'rb') as image_fd:
        boot_magic = unpack('8s', image_fd.read(8))[0].decode()
        image_fd.seek(0)
        if boot_magic == 'ANDROID!':
            info = unpack_boot_image(image_fd, output_dir)
        elif boot_magic == 'VNDRBOOT':
            info = unpack_vendor_boot_image(image_fd, output_dir)
        else:
