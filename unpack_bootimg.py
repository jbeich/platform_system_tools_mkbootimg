...
def unpack_bootimage(args):
    ...
    if version >= 4:
        boot_digest_size = unpack('I', args.boot_img.read(4))[0]
        print('boot.img digest size: %s' % boot_digest_size)
    else:
        boot_digest_size = 0
    ...
    if boot_digest_size > 0:
        # boot digest only exists in boot.img version >= v4.
        # There are only kernel and ramdisk pages before the digest.
        boot_digest_offset = page_size * (
            num_header_pages + num_kernel_pages + num_ramdisk_pages)

        image_info_list.append((boot_digest_offset, boot_digest_size,
                                'gki_vbmeta'))
