...
def unpack_bootimage(args):
    ...
    if version < 3:
        ...
        print('os version and patch level: %s' % kernel_ramdisk_second_info[3])
...
def unpack_vendor_bootimage(args):
    kernel_ramdisk_second_info = unpack('5I', args.boot_img.read(5 * 4))
    print('vendor boot image header version: %s' % kernel_ramdisk_second_info[0])
    print('kernel load address: %#x' % kernel_ramdisk_second_info[2])
    print('ramdisk load address: %#x' % kernel_ramdisk_second_info[3])
    print('vendor ramdisk size: %s' % kernel_ramdisk_second_info[4])

    cmdline = unpack('2048s', args.boot_img.read(2048))[0].decode()
    print('vendor command line args: %s' % cmdline)

    kernel_ramdisk_third_info = unpack('I', args.boot_img.read(1 * 4))
    print('kernel tags load address: %#x' % kernel_ramdisk_third_info[0])
    ...
    ramdisk_size = kernel_ramdisk_second_info[4]
    page_size = kernel_ramdisk_second_info[1]
