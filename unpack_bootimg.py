...
def unpack_bootimage_v3(args):
    ...
    vendor_page_size = vendor_kernel_ramdisk_second_info[1]

    # The first page contains the boot header
    num_boot_header_pages = 1

    num_boot_kernel_pages = get_number_of_pages(kernel_size, BOOT_IMAGE_HEADER_V3_PAGESIZE)
    kernel_offset = BOOT_IMAGE_HEADER_V3_PAGESIZE * num_boot_header_pages  # header occupies a page
    image_info_list = [(kernel_offset, kernel_size, 'kernel')]

    num_boot_ramdisk_pages = get_number_of_pages(ramdisk_size, BOOT_IMAGE_HEADER_V3_PAGESIZE)
    ramdisk_offset = BOOT_IMAGE_HEADER_V3_PAGESIZE * (num_boot_header_pages + num_boot_kernel_pages
                                 ) # header + kernel
    image_info_list.append((ramdisk_offset, ramdisk_size, 'ramdisk'))
    ...
    num_vendor_boot_ramdisk_pages = get_number_of_pages(vendor_ramdisk_size, vendor_page_size)
    vendor_ramdisk_offset = vendor_page_size * num_vendor_boot_header_pages # header occupies a page
    vendor_image_info_list = [(vendor_ramdisk_offset, vendor_ramdisk_size, 'vendor_ramdisk')]

    num_vendor_boot_dtb_pages = get_number_of_pages(dtb_size, vendor_page_size)
