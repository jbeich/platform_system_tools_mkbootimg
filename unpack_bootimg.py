...
class BootImageInfoFormatter:
    ...
    def format_pretty_text(self):
        ...
        if self.header_version < 3:
            ...
            lines.append(f'kernel load address: {self.kernel_load_address:#x}')
            lines.append(f'ramdisk size: {self.ramdisk_size}')
            lines.append(
                f'ramdisk load address: {self.ramdisk_load_address:#x}')
            lines.append(f'second bootloader size: {self.second_size}')
            lines.append(
                f'second bootloader load address: '
                f'{self.second_load_address:#x}')
            lines.append(
                f'kernel tags load address: {self.tags_load_address:#x}')
        ...
        if 0 < self.header_version < 3:
            lines.append(f'recovery dtbo size: {self.recovery_dtbo_size}')
            lines.append(
                f'recovery dtbo offset: {self.recovery_dtbo_offset:#x}')
            lines.append(f'boot header size: {self.boot_header_size}')

        if 1 < self.header_version < 3:
            lines.append(f'dtb size: {self.dtb_size}')
            lines.append(f'dtb address: {self.dtb_load_address:#x}')

        if self.header_version >= 4:
            lines.append(f'boot.img signature size: {self.boot_signature_size}')
