...
def unpack_vendor_bootimage(args):
    ...
    if args.format == 'mkbootimg':
        print(info.format_mkbootimg_argument())
    else:
...
def get_unpack_usage():
  ...
  * info
    ...
    Pretty-printed info-rich text format suitable for human consumption.

  * mkbootimg

    Argument strings that can be used to reconstruct the boot image with
    mkbootimg. For example,

    $ unpack_bootimg --boot_img vendor_boot.img --out out --format=mkbootimg |
      tee mkbootimg_args
    $ sh -c "mkbootimg $(cat mkbootimg_args) --vendor_boot vendor_boot.rebuilt"

    vendor_boot.img and vendor_boot.rebuilt would be equivalent.
"""
