...
impl Default for BootImgHdrV0 {
    fn default() -> Self {
        Self {
            magic: BOOT_MAGIC.try_into().unwrap(),
            kernel_size: 0,
            kernel_addr: 0,
            ramdisk_size: 0,
            ramdisk_addr: 0,
...
impl<B: ByteSlice + PartialEq> BootImg<B> {
    ...
    pub fn parse_boot_image(buffer: B) -> BootResult<Self> {
        ...
        if (&buffer)[0..BOOT_MAGIC.len()].ne(BOOT_MAGIC) {
...
pub struct VendorBootHdrV3 {
    ...
    pub magic: [u8; VENDOR_BOOT_MAGIC.len()],
...
impl Default for VendorBootHdrV3 {
    fn default() -> Self {
        Self {
            magic: VENDOR_BOOT_MAGIC.try_into().unwrap(),
            header_version: 3,
            page_size: 0,
            kernel_addr: 0,
            ramdisk_addr: 0,
...
impl<B: ByteSlice + PartialEq> VendorBootHdr<B> {
    ...
    pub fn parse_vendor_boot_image(buffer: B) -> BootResult<Self> {
        // TODO(dovs): when core::offset_of has stabilized, use that instead of raw sizes
        let version_end_offset = VENDOR_BOOT_MAGIC.len() + size_of::<u32>();
        ...
        // In all headers, the first 8 bytes are the magic string.
        if (&buffer)[0..VENDOR_BOOT_MAGIC.len()].ne(VENDOR_BOOT_MAGIC) {
            return Err(BootError::BadMagic);
        }

        let version = u32::from_le_bytes(
            (&buffer)[VENDOR_BOOT_MAGIC.len()..version_end_offset]
                .try_into()
...
mod tests {
    ...
    fn bad_magic() {
        ...
        add::<BootImgHdrV0>(
            &mut buffer,
            BootImgHdrV0 { magic: *b"ANDROGEN", ..Default::default() },
        );
    ...
    fn vendor_buffer_too_small_for_version() {
        let buffer = [0; VENDOR_BOOT_MAGIC.len() + 3];
        assert_eq!(
    ...
    fn vendor_bad_magic() {
        ...
        add::<VendorBootHdrV3>(
            &mut buffer,
            VendorBootHdrV3 { magic: *b"VNDRBOOK", ..Default::default() },
        );
