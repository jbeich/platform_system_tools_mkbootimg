...
def write_vendor_boot_header(args):
    ...
    VENDOR_RAMDISK_TABLE_ENTRY_V4_SIZE = 72
...
class VendorRamdiskTableBuilder:
    ...
    VendorRamdiskTableEntry = collections.namedtuple(
        ...
        ['ramdisk_path', 'ramdisk_size', 'ramdisk_offset', 'board_id'])

    def __init__(self):
        self.entries = []
        self.ramdisk_total_size = 0

    def add_entry(self, ramdisk_path, board_id=None):
        if board_id is None:
        ...
        self.entries.append(self.VendorRamdiskTableEntry(
            ramdisk_path, ramdisk_size, self.ramdisk_total_size, board_id))
...
def parse_vendor_ramdisk_args(args_list, args):
    """Parses vendor ramdisk specific arguments.

    Args:
        args_list: A list of argument strings.
    ...
    while VENDOR_RAMDISK_FRAGMENT_OPTION in args_list:
        ...
        board_id = [ramdisk_args_dict['board_id{}'.format(i)]
                    for i in range(VENDOR_RAMDISK_TABLE_ENTRY_BOARD_ID_SIZE)]
        vendor_ramdisk_table_builder.add_entry(ramdisk_path, board_id)
...
def main():
    ...
    if args.vendor_boot is not None:
        ...
        if args.header_version > 3:
            extra_args = parse_vendor_ramdisk_args(extra_args, args)
