...
VENDOR_RAMDISK_KIND_NONE = 0
VENDOR_RAMDISK_KIND_PLATFORM = 1
VENDOR_RAMDISK_KIND_RECOVERY = 2
VENDOR_RAMDISK_KIND_VENDOR = 3
...
class VendorRamdiskTableBuilder:
    ...
    VendorRamdiskTableEntry = collections.namedtuple(
        ...
        ['ramdisk_path', 'ramdisk_size', 'ramdisk_offset', 'ramdisk_kind', 'board_id'])

    def __init__(self):
        self.entries = []
        self.ramdisk_total_size = 0

    def add_entry(self, ramdisk_path, ramdisk_kind=VENDOR_RAMDISK_KIND_NONE, board_id=None):
        if board_id is None:
        ...
        self.entries.append(self.VendorRamdiskTableEntry(
            ramdisk_path, ramdisk_size, self.ramdisk_total_size, ramdisk_kind, board_id))
    ...
    def write_entries_padded(self, fout, alignment):
        for entry in self.entries:
            ...
            fout.write(pack('I', entry.ramdisk_kind))
...
def parse_vendor_ramdisk_kind(x):
    kind_dict = {
        'none': VENDOR_RAMDISK_KIND_NONE,
        'platform': VENDOR_RAMDISK_KIND_PLATFORM,
        'recovery': VENDOR_RAMDISK_KIND_RECOVERY,
        'vendor': VENDOR_RAMDISK_KIND_VENDOR,
    }
    if x.lower() in kind_dict:
        return kind_dict[x.lower()]
    return parse_int(x)


def get_vendor_boot_v4_usage():
    return """vendor boot version 4 arguments:
  --ramdisk_kind {none,platform,recovery,vendor}
                        specify the kind of the ramdisk
...
def parse_vendor_ramdisk_args(args, args_list):
    ...
    parser.add_argument('--ramdisk_kind', type=parse_vendor_ramdisk_kind,
                        default=VENDOR_RAMDISK_KIND_NONE)
    ...
    while VENDOR_RAMDISK_FRAGMENT_OPTION in args_list:
        ...
        ramdisk_kind = ramdisk_args.ramdisk_kind
        board_id = [ramdisk_args_dict['board_id{}'.format(i)]
                    for i in range(VENDOR_RAMDISK_TABLE_ENTRY_BOARD_ID_SIZE)]
        vendor_ramdisk_table_builder.add_entry(ramdisk_path, ramdisk_kind, board_id)
