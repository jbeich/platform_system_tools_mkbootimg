...
class RamdiskImage:
    ...
    def _unpack_ramdisk(self):
        ...
        decompressed_ramdisk = self._temp_file_manager.make_temp_file(
            prefix='ramdisk-decompressed')
        ...
        for compression_type, compression_util in [
            ...
            decompression_cmd = '%s -d -c %s > %s' % (
                compression_util, self._ramdisk_img, decompressed_ramdisk)

            decompressed_result = subprocess.run(
                decompression_cmd, shell=True, check=False,
        ...
        if self._ramdisk_format is not None:
            extract_cpio_cmd = 'toybox cpio -idm < %s' % decompressed_ramdisk
            ...
            cpio_result = subprocess.run(extract_cpio_cmd, shell=True,
