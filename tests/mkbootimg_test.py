    ...
    def test_unpack_boot_image_v3_json_args(self):
        ...
        with tempfile.TemporaryDirectory() as temp_out_dir:
            ...
            with open (json_file) as json_fd:
                ...
                if actual_mkbootimg_args != expected_mkbootimg_args:
                    msg = '\n'.join([
                        'Unexpected content in mkbootimg_args.json:',
                        'Expected:',
                        ' ' + json.dumps(expected_mkbootimg_args, sort_keys=True),
                        '',
                        'Actual:',
                        ' ' + json.dumps(actual_mkbootimg_args, sort_keys=True)
                    ])
                    self.fail(msg)
