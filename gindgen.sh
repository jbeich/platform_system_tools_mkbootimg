...
./out/host/linux-x86/bin/bindgen --blocklist-type="${BLOCKED_TYPES_RE}" \
...
cat << EOF | cat - ${SCRATCH_DIR}/bootimg_gen.rs > ${BOOTIMG_DIR}/src/bootimg_priv.rs
...
rustfmt ${BOOTIMG_DIR}/src/bootimg_priv.rs --config-path system/tools/aidl/rustfmt.toml
