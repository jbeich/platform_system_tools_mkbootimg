...
readonly RETROFITTED_V3_BOOT_SIGNATURE_SIZE=$(( 16 << 10 ))
readonly RETROFITTED_V2_BOOT_SIGNATURE_SIZE=$(( 16 << 10 ))
...
if [[ "${OUTPUT_BOOT_IMAGE_VERSION}" -eq 4 ]]; then
  ...
  ( [[ -n "${VERBOSE}" ]] && set -x
    ...
    if [[ "$(file_size "${OUTPUT_BOOT_SIGNATURE}")" -gt "${RETROFITTED_V3_BOOT_SIGNATURE_SIZE}" ]]; then
      die "boot signature size is larger than ${RETROFITTED_V3_BOOT_SIGNATURE_SIZE}"
    fi
    truncate "${OUTPUT_BOOT_SIGNATURE}" -s "${RETROFITTED_V3_BOOT_SIGNATURE_SIZE}"
  ...
  if [[ -f "${VENDOR_BOOT_DIR}/dtb" ]]; then
    repack_args+=(--dtb "${VENDOR_BOOT_DIR}/dtb")
  fi

  ( [[ -n "${VERBOSE}" ]] && set -x
    "${MKBOOTIMG}" "${repack_args[@]}" --output "${OUTPUT_BOOT_IMAGE}"

    # Pad the boot signature and append it to the end.
    if [[ "$(file_size "${OUTPUT_BOOT_SIGNATURE}")" -gt "${RETROFITTED_V2_BOOT_SIGNATURE_SIZE}" ]]; then
      die "boot signature size is larger than ${RETROFITTED_V2_BOOT_SIGNATURE_SIZE}"
    fi
    truncate "${OUTPUT_BOOT_SIGNATURE}" -s "${RETROFITTED_V2_BOOT_SIGNATURE_SIZE}"
    cat "${OUTPUT_BOOT_SIGNATURE}" >> "${OUTPUT_BOOT_IMAGE}"
  )
fi
