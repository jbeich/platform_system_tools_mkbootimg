...
def get_kernel(boot_img):
    ...
    with tempfile.TemporaryDirectory() as unpack_dir:
        ...
        subprocess.run(unpack_bootimg_cmd, check=True, capture_output=True)

        with tempfile.NamedTemporaryFile(delete=False) as kernel_file:
            kernel_file.close()
            shutil.copy2(os.path.join(unpack_dir, 'kernel'),
                         kernel_file.name)

            assert os.path.getsize(kernel_file.name) > 0
