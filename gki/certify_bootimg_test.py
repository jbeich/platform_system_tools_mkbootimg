...
class CertifyBootimgTest(unittest.TestCase):
    ...
    def test_certify_bootimg_with_avb_footer(self):
        """Tests the avb_footer location remains after certify_bootimg."""
        with tempfile.TemporaryDirectory() as temp_out_dir:
            boot_img = os.path.join(temp_out_dir, 'boot.img')
            generate_test_boot_image(boot_img=boot_img,
                                     avb_partition_size=128 * 1024)
            assert has_avb_footer(boot_img)
            ...
            assert has_avb_footer(boot_certified_img)
            assert (os.path.getsize(boot_img) ==
    ...
    def test_certify_bootimg_with_avb_footer_and_with_another_key(self):
        """Tests re-certify a boot image with AVB again with a different key."""
        with tempfile.TemporaryDirectory() as temp_out_dir:
            boot_img = os.path.join(temp_out_dir, 'boot.img')
            generate_test_boot_image(boot_img=boot_img,
                                     avb_partition_size=128 * 1024)
            ...
            assert has_avb_footer(boot_certified2_img)
            assert (os.path.getsize(boot_certified_img) ==
