...
#define VENDOR_RAMDISK_KIND_NONE 0
#define VENDOR_RAMDISK_KIND_PLATFORM 1
#define VENDOR_RAMDISK_KIND_RECOVERY 2
#define VENDOR_RAMDISK_KIND_VENDOR 3
 ...
 * ramdisk is tagged with a kind value and a set of hardware identifiers
 ...
 *
 * The vendor ramdisk table holds the size, offset, kind and hardware
 * identifiers of each ramdisk. The kind field denotes the type of the ramdisk.
 ...
 *
 * For the different kind of ramdisks, there are:
 *    - VENDOR_RAMDISK_KIND_NONE indicates the value is unspecified.
 *    - VENDOR_RAMDISK_KIND_PLATFORM ramdisk contains platform specific bits.
 *    - VENDOR_RAMDISK_KIND_RECOVERY ramdisk contains recovery resources.
 *    - VENDOR_RAMDISK_KIND_VENDOR ramdisk contains vendor specific bits, such
 *      as dynamic loadable kernel modules.
 *
 * 0. all entities in the boot image are 4096-byte aligned in flash, all
 *    entities in the vendor boot image are page_size (determined by the vendor
 *    and specified in the vendor boot image header) aligned in flash
...
struct vendor_ramdisk_table_entry_v4 {
    ...
    uint32_t ramdisk_kind; /* kind of the ramdisk */
