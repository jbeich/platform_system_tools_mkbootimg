 ...
 * o = (vendor_bootconfig_size + page_size - 1) / page_size
 *
 ...
 * entering the kernel.
 * FIXME do we want to link to the external bootconfig site for param and trailer format?
 *
 * 0. all entities in the boot image are 4096-byte aligned in flash, all
 *    entities in the vendor boot image are page_size (determined by the vendor
 *    and specified in the vendor boot image header) aligned in flash
 ...
 * 4. load the generic ramdisk immediately following the vendor ramdisk in
 *    memory
 * 5. load the bootconfig immediately following the boot image ramdisk. Add
...
struct vendor_boot_img_hdr_v4 : public vendor_boot_img_hdr_v3 {
    ...
    uint32_t bootconfig_size; /* size in bytes for the boot config section */
} __attribute__((packed));
