 ...
 * ramdisk is tagged with a set of hardware identifiers describing the board,
 * soc or platform that this ramdisk is created for.
 *
 * The vendor ramdisk section is consist of multiple ramdisk images concatenated
 * one after another, and vendor_ramdisk_size is the size of the section, which
 * is the total size of all the ramdisks included in the vendor boot image.
 *
 * The vendor ramdisk table holds the size, offset and hardware identifiers of
 * each ramdisk. The hardware identifiers are specified in the board_id field of
 * each table entry. The board_id field is consist of a vector of unsigned
 * integer words, and the encoding scheme is defined by the hardware vendor.
 *
 * 0. all entities in the boot image are 4096-byte aligned in flash, all
 *    entities in the vendor boot image are page_size (determined by the vendor
 *    and specified in the vendor boot image header) aligned in flash
 ...
 *    jump to kernel_addr
 */
typedef struct boot_img_hdr_v3 boot_img_hdr_v4;
